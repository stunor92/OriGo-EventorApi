...
import no.stunor.origo.eventorapi.model.event.resultlist.PersonResult
import no.stunor.origo.eventorapi.model.event.resultlist.TeamMemberResult
import no.stunor.origo.eventorapi.model.event.resultlist.TeamResult
...
class ResultListConverter {
    ...
    fun convertEventResultList(eventor: Eventor, resultList: ResultList): List<CompetitorResult> {
        val competitorList: MutableList<CompetitorResult> = mutableListOf()
    ...
    @Throws(NumberFormatException::class, ParseException::class)
    private fun convertOneDayPersonResult(eventor: Eventor, event: Event, classResult: org.iof.eventor.ClassResult, personResult: org.iof.eventor.PersonResult): CompetitorResult {
        return PersonResult(
    ...
    private fun convertMultiDayPersonResult(
        ...
        raceResult: org.iof.eventor.RaceResult): CompetitorResult {
        return PersonResult(
    ...
    fun convertTeamMembers(eventor: Eventor, teamMembers: List<org.iof.eventor.TeamMemberResult>): List<TeamMemberResult> {
        val result: MutableList<TeamMemberResult> = mutableListOf()
    ...
    @Throws(ParseException::class)
    private fun convertTeamMember(eventor: Eventor, teamMember: org.iof.eventor.TeamMemberResult): TeamMemberResult {
    ...
    private fun convertOverallResult(overallResult: org.iof.eventor.OverallResult): Result? {
        if(overallResult.teamStatus == null || overallResult.teamStatus.value == "Inactive")
            return null
        return Result(
    ...
    private fun convertLegResult(teamMember: org.iof.eventor.TeamMemberResult): Result? {
        if(teamMember.competitorStatus == null || teamMember.competitorStatus.value == "Inactive")
            return null
        return Result(
